#!chezscheme
;;; std-crypto-digest.sls -- Compat shim for Gerbil's :std/crypto/digest
;;; Uses openssl CLI for hash computation

(library (compat std-crypto-digest)
  (export
  ...
  (define (compute-digest algo data)
    ...
    (let* ((input (if (bytevector? data) data (string->utf8 data)))
      ...
      (let ((tmp-file (format "/tmp/gherkin-digest-~a" (random 1000000))))
