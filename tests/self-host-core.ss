...
;; Load all .ss module paths from a file (one per line)
(define (read-module-list path)
...
;; Build module list: either from file or inline fallback
(define loader-test-modules
  (guard (exn [#t
    ...
    '(":std/misc/queue" ":std/misc/deque" ":std/misc/pqueue"
      ...
      ":std/getopt" ":std/cli/getopt" ":std/logger" ":std/deprecation"
      ":std/os/signal" ":std/os/signal-handler" ":std/os/fdio"
      ":std/os/path" ":std/os/env" ":std/os/temporaries"
      ":std/coroutine" ":std/event" ":std/io"
      ":std/net/httpd" ":std/net/request" ":std/net/uri"
      ":std/crypto" ":std/crypto/digest"
      ":std/xml" ":std/markup/xml"
      ":std/build-script" ":std/build" ":std/cli/multicall")])
    ;; Try to use the full list generated by find — includes both std/ and gerbil/ modules
    (if (file-exists? "/tmp/all-gherkin-test-modules.txt")
      (read-module-list "/tmp/all-gherkin-test-modules.txt")
      ;; Generate it: find all .ss files in both std/ and gerbil/ trees
      (begin
        (system (string-append
          "{ find " gerbil-dir-i3 "/src/std -name '*.ss' "
          "-not -name '*-test.ss' -not -name '*-benchmark*' ; "
          "find " gerbil-dir-i3 "/src/gerbil -name '*.ss' "
          "-not -name '*-test.ss' ; } "
          "| while read f; do echo \"$f\" | sed \"s|" gerbil-dir-i3 "/src/|:|;s|\\.ss||\"; done "
          "| sort > /tmp/all-gherkin-test-modules.txt"))
        (read-module-list "/tmp/all-gherkin-test-modules.txt")))))
