...
A gherkin project wraps an existing Gerbil project with build scripts and a
compatibility layer:
...
my-gherkin-project/
  ...
  build-gherkin.ss           # Gherkin translation driver
...
.PHONY: all gherkin compile binary run clean

all: gherkin compile

# Translate .ss -> .sls via gherkin
gherkin:
...
The `GHERKIN` variable points to gherkin's `src/` directory. Override it:
```bash
GHERKIN_DIR=/path/to/gherkin/src make
```

If your project depends on other gherkin-compiled libraries, add them to
`LIBDIRS`:
```makefile
GHERKIN_AWS = $(or $(GHERKIN_AWS_DIR),gherkin-aws/src)
LIBDIRS = src:$(GHERKIN):$(GHERKIN_AWS)
```

## Step 2: Write the Import Map

The import map is the most important part. It tells gherkin how to translate
...
(define my-import-map
  '(;; Standard library -> compat shims
    ...
    (:std/iter         . #f)   ;; gherkin compiles for-loops natively
    ...
    (:ober/aws/s3      . (gherkin-aws s3-api))
    ))
```
...
- **`gerbil-compile-to-library`** `(input-path lib-name import-map base-imports)`
  Reads a `.ss` file, compiles it through gherkin, wraps in an R6RS library form.
...
You don't always need to write these from scratch. Check if gherkin-shell's
...
**Problem**: Gerbil uses `key: value` keyword arguments. Gherkin translates
these, but `defclass` constructors may pass keyword values as positional args.

**Fix**: Post-build patch in `build-gherkin.ss`:
```scheme
...
`memfd` + `Sscheme_script`. See [docs/gherkin-single-binary.md](gherkin-single-binary.md).
...
[docs/gherkin-single-binary.md](gherkin-single-binary.md).
...
|---------|---------|--------------|
| [gherkin-shell](https://github.com/ober/gherkin-shell) | 30+ | FFI shim, binary building, post-build patches |
| gherkin-kunabi | 10+ | Cross-project deps (gherkin-aws), sed-based patches |
| gherkin-lsp | 53 | Large module count, JSON/HTTP compat |
