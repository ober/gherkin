...
This document captures every trick used to build gherkin-shell's `gsh` — a single ELF binary that embeds Chez Scheme's runtime, boot files, 30+ Gerbil-to-Chez compiled modules, a Gerbil reader/compiler, and POSIX FFI bindings. The binary works from any directory with zero external dependencies (beyond libc and system libraries).
...
6. [The Gherkin Compilation Pipeline](#6-the-gherkin-compilation-pipeline)
...
```
┌──────────────────────────────────────────┐
│  gsh-main.c (custom C entry point)       │
├──────────────────────────────────────────┤
│  petite.boot  ─┐                         │
│  scheme.boot   ├─ C byte arrays          │
│  gsh.boot     ─┘  (Sregister_boot_file_bytes) │
├──────────────────────────────────────────┤
│  gsh_program.h  (compiled program .so    │
│                   as C byte array,       │
│                   loaded via memfd)       │
├──────────────────────────────────────────┤
...
```c
// gsh_petite_boot.h — ~1.9 MB as C array
...
The boot files total ~5.5 MB (petite ~1.9 MB, scheme ~1.0 MB, gsh ~2.6 MB), all compiled into the binary's `.rodata` section.
...
(apply make-boot-file "gsh.boot" '("scheme" "petite")
  (append
    gherkin-runtime-modules    ; types, MOP, error, hash, compiler...
    compat-layer-modules       ; gambit shims, misc, sugar, pregexp...
    gsh-modules))              ; all shell modules in dependency order

;; Program is compiled separately
(compile-program "gsh.ss")     ; produces gsh.so
```
...
write(fd, gsh_program_data, gsh_program_size);
...
**Solution**: Write a custom `gsh-main.c` that saves all arguments in numbered environment variables before Chez sees them:

```c
// C main saves args in env vars
setenv("GSH_ARGC", "3", 1);
setenv("GSH_ARG0", "-c", 1);
setenv("GSH_ARG1", "echo hello", 1);
setenv("GSH_ARG2", "world", 1);
      ...
      (let ((argc (string->number argc-str)))
        (let loop ((i 0) (acc '()))
          (if (>= i argc) (reverse acc)
            (loop (+ i 1) (cons (getenv (format "GSH_ARG~a" i)) acc)))))
...
```
gcc -rdynamic -o gsh gsh-main.o ffi-shim.o -lkernel -llz4 -lz -lm -ldl -lpthread -luuid -lncurses
```

---

## 6. The Gherkin Compilation Pipeline
...
**Solution**: The Gherkin compiler (`gerbil-compile-to-library`) transforms each `.ss` file into an R6RS `(library ...)` form. An import map translates Gerbil module references to Chez library names:
...
(define gsh-import-map
  '((:std/sugar        . (compat sugar))     ; Gerbil sugar → our compat lib
    ...
    (:std/iter         . #f)                  ; stripped — Gherkin handles natively
    (:std/foreign      . #f)                  ; stripped — not needed on Chez
    (:gerbil/core      . #f)                  ; stripped
    ("./pregexp-compat" . (gsh pregexp-compat)) ; relative import
    ...))
```
...
**Problem**: R6RS forbids local definitions that shadow imported bindings. After Gherkin compilation, many modules define names that conflict with their imports (e.g., a module defines `find` which conflicts with `(compat misc)`'s `find`).
...
**Solution**: Inject a dummy reference in `main.sls` that forces Chez to invoke `(gsh builtins)`:

```scheme
;; Patched into main.sls after Gherkin compilation:
(define _force-builtins special-builtin?)  ; references a builtins export
```

The entry point `gsh.ss` also forces it:
```scheme
(let () special-builtin? (void))  ; force (gsh builtins) invocation
```
...
After Gherkin compilation, several patterns in the generated `.sls` files need fixing. These are applied as string-level patches on the generated code:

### 10a. Keyword Dispatch in defclass Constructors

**Bug**: Gherkin's `defclass` constructor translation incorrectly appends keyword argument *values* to the positional argument list. The generated `init!` method is a `case-lambda` dispatching on positional arity, so extra values cause the wrong clause to match.
...
**Bug**: Gerbil's `void` is variadic (accepts any args, returns void). Gherkin translates `(define (void) (void))` which creates a 0-arg function that infinitely recurses.
...
**Problem**: Chez's Whole-Program Optimization (WPO) analyzes all code and eliminates dead definitions. The `identifier-syntax` macros generated for mutable exports (see §8) appear to WPO as unused macro-only definitions — it eliminates the underlying vector-cell storage, breaking everything.

**Solution**: Skip WPO entirely. Use the plain `gsh.so` from `compile-program` without running `compile-whole-program`:

```scheme
;; DO NOT use compile-whole-program — it eliminates identifier-syntax cells
(compile-program "gsh.ss")    ; produces gsh.so
(system "cp gsh.so gsh-all.so")  ; use directly, no WPO pass
```
...
The `make-boot-file` call must list `.so` files in strict dependency order. A module can only reference definitions from modules listed before it. The order for gherkin-shell:

```
1. Gherkin runtime (9 modules):
   types → gambit-compat → util → table → c3 → mop → error → hash
   → syntax → eval → reader → compile → gherkin

2. Compat layer (9 modules):
   gambit → misc → sort → sugar → format → pregexp
   → signal → signal-handler → fdio

3. GSH modules (30 modules, 7 tiers):
...
```
Step 1: gcc → libgsh-ffi.so          (C FFI shim, shared lib for interpreted mode)
Step 2: Gherkin → src/gsh/*.sls       (30 .ss files → 30 .sls R6RS libraries)
        + post-build patches           (import conflicts, mutable exports, Chez compat)
Step 3: Chez compile-program          (gsh.ss → gsh.so, compiles all imported libs)
Step 4: make-boot-file                (48 .so files → gsh.boot, libs-only)
Step 5: file→c-header × 4            (petite.boot, scheme.boot, gsh.boot, gsh.so → .h)
Step 6: gcc → gsh-main.o             (custom main with embedded boot+program arrays)
        gcc → ffi-shim.o             (POSIX bindings)
Step 7: gcc → gsh                    (link with -lkernel -llz4 -lz -lm -ldl -lpthread -luuid -lncurses)

Result: ./gsh  (~6.5 MB, fully self-contained ELF)
```
...
| gsh.boot | ~2.6 MB | Gherkin runtime + compat + 30 shell modules |
| gsh program | ~9 KB | Entry point + Gerbil eval handler |
