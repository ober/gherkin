# Things I Learned: Converting gerbil-lsp to gherkin-lsp

Notes from porting a 53-module Gerbil Scheme LSP server to run on Chez Scheme
via the Gherkin translation framework. Covers bugs found, compatibility issues,
and design decisions.

## Gherkin Framework Bugs Found and Fixed
...
Gerbil's `#:parse-error` keyword literal syntax is not recognized by the Gherkin
...
### Chez `hashtable` vs Gherkin Runtime Hash Tables

The Gherkin runtime wraps hash tables in its own `gerbil-struct` record type.
Chez has a native `hashtable` type. If a compat module creates hash tables with
Chez primitives (`make-hashtable`), they crash when passed to Gherkin's
`hash-ref` / `hash-put!`.
...
- `keyword?` — checks if symbol ends with `:` (Gherkin's keyword convention)
- `type-of` — runtime type dispatch returning a symbol

### FFI Stubs in `gambit.sls`

The gambit.sls from gherkin-shell had `foreign-procedure` calls for process and
terminal management (`ffi_do_waitpid`, `ffi_set_raw_mode`, etc.). These required
gherkin-shell's C shared library. Replaced with no-op stubs since the LSP server
doesn't need them.
...
1. **Gherkin translate** (`.ss` → `.sls`): `build-gherkin.ss` drives the Gherkin
...
- 4 Gherkin framework bugs fixed upstream
